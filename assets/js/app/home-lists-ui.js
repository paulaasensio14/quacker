    ...
    if (markBtn) {
      ...
      if (!lastActivity?.id) {
        markBtn.innerHTML = `
          ...
          ${window.I18n.t("home_last_activity_button_idle")}
        `;
        markBtn.disabled = true;
        markBtn.classList.remove("completed");
      ...
      if (!backlogUnique || !backlogUnique.length) {
        ...
        backlogContainer.innerHTML = backlogUnique
          .map((item) => {
            ...
            if (isHot) {
              backlogChip = `<span class="backlog-chip chip-hot">${days} ${window.I18n.t(dayKey)}</span>`;
            } else if (isWarm) {
              backlogChip = `<span class="backlog-chip chip-warm">${days} ${window.I18n.t(dayKey)}</span>`;
            }
